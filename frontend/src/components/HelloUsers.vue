...
xhr.send(data);
  ...
  mounted(){
    ...
    // fetch("http://localhost:3000/showusers").then( res => res.json() ).then( jsonData => this.allUsers = jsonData );
     fetch("http://localhost:3000/showusers").then( res => console.log( res ) );
     fetch("http://localhost:3000/showusers").then( res => res.json() ).then (jsonData => this.allUsers = jsonData );
